...
// AuthorHead creates a header using the author field.
// The bot's avatar is included.
func (e *Embed) AuthorHead(header string, link ...string) *Embed {
	realLink := ""
	if len(link) > 0 {
		realLink = link[0]
	}
	...
	e.Author = &discordgo.MessageEmbedAuthor{
		Name: header,
		IconURL: e.context.Session.State.User.AvatarURL(""),
		URL: realLink,
	}
...
// RoleColor sets the embed color to the bot's role color.
// If there is none, use white.
func (e *Embed) RoleColor() *Embed {
	c := e.context
	...
	member, err := c.Session.State.Member(channel.GuildID, c.Session.State.User.ID)
	if err != nil {
		panic(err)
	}

	for _, roleID := range member.Roles {
		role, err := c.Session.State.Role(channel.GuildID, roleID)
		if err != nil {
			continue
		}

		if role.Color != 0x1FFFFFFF {
			e.Color = role.Color
			return e
		}
	}
